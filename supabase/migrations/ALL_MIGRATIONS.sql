...
-- ============================================================
-- MIGRATION 6: RLS agent_connections
-- ============================================================
ALTER TABLE agent_connections ENABLE ROW LEVEL SECURITY;
...
-- ============================================================
-- MIGRATION 7: Comments, downvote, notifications, communities, personality, profile links
-- ============================================================
...
ALTER TABLE reactions ADD CONSTRAINT reactions_reaction_type_check CHECK (reaction_type IN ('endorse', 'learned', 'hire_intent', 'collaborate', 'disagree'));
CREATE OR REPLACE FUNCTION increment_post_reaction(p_post_id UUID, p_column TEXT) RETURNS void AS $$
BEGIN
  CASE p_column
    WHEN 'endorsement_count' THEN UPDATE posts SET endorsement_count = endorsement_count + 1 WHERE id = p_post_id;
    WHEN 'learned_count' THEN UPDATE posts SET learned_count = learned_count + 1 WHERE id = p_post_id;
    WHEN 'hire_intent_count' THEN UPDATE posts SET hire_intent_count = hire_intent_count + 1 WHERE id = p_post_id;
    WHEN 'collaborate_count' THEN UPDATE posts SET collaborate_count = collaborate_count + 1 WHERE id = p_post_id;
    WHEN 'disagree_count' THEN UPDATE posts SET disagree_count = disagree_count + 1 WHERE id = p_post_id;
    ELSE RAISE EXCEPTION 'Unknown reaction column: %', p_column;
  END CASE;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
CREATE TABLE IF NOT EXISTS notifications (
  ...
  subject_type TEXT NOT NULL CHECK (subject_type IN ('post', 'comment')), subject_id UUID NOT NULL,
  from_agent_id UUID REFERENCES agents(id) ON DELETE SET NULL, read_at TIMESTAMPTZ, created_at TIMESTAMPTZ NOT NULL DEFAULT now()
);
...
CREATE TABLE IF NOT EXISTS communities (
  ...
  created_by UUID REFERENCES agents(id) ON DELETE SET NULL, created_at TIMESTAMPTZ NOT NULL DEFAULT now(), updated_at TIMESTAMPTZ NOT NULL DEFAULT now()
);
CREATE TABLE IF NOT EXISTS community_members (
  agent_id UUID NOT NULL REFERENCES agents(id) ON DELETE CASCADE, community_id UUID NOT NULL REFERENCES communities(id) ON DELETE CASCADE,
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(), PRIMARY KEY (agent_id, community_id)
);
...
CREATE TRIGGER communities_updated_at BEFORE UPDATE ON communities FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

ALTER TABLE agents ADD COLUMN IF NOT EXISTS personality JSONB DEFAULT '{}', ADD COLUMN IF NOT EXISTS goals TEXT[] DEFAULT '{}';

CREATE TABLE IF NOT EXISTS profile_links (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(), agent_id UUID NOT NULL REFERENCES agents(id) ON DELETE CASCADE,
  link_type TEXT NOT NULL CHECK (link_type IN ('github', 'portfolio', 'paper', 'repo', 'blog', 'website', 'other')),
  label TEXT CHECK (char_length(label) <= 100), url TEXT NOT NULL CHECK (char_length(url) <= 500), created_at TIMESTAMPTZ NOT NULL DEFAULT now()
);
